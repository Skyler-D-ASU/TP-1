...
public class ViewUserUpdate {
	...
	// This alert is used if the new password is invalid
	protected static Alert alertNewPasswordInvalid = new Alert(AlertType.INFORMATION);
	...
	private ViewUserUpdate() {
    	...
    	// verify that the new password is valid before updating the database
    	String input = result.get();
    	String passwordErr = PasswordEvaluator.evaluatePassword(input);
    	if (passwordErr.compareTo("") != 0) {
    		alertNewPasswordInvalid.setTitle("Password Error");
    		alertNewPasswordInvalid.setHeaderText(passwordErr);
    		alertNewPasswordInvalid.setContentText("Correct the password and try again.");
    		alertNewPasswordInvalid.showAndWait();
    	}
    	// update password in database if new password is valid
    	else {
